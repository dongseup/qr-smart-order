...
async function testMenuCRUD() {
  ...
  try {
    ...
    console.log(
      `   ✅ 메뉴 수정 성공: 가격 ${testMenu.price}원 → ${updated.price}원`
    );
    console.log("");
...
async function testTransaction() {
  ...
  try {
    ...
    console.log(
      `   ✅ 트랜잭션 성공: ${result.menu1.name}, ${result.menu2.name}`
    );
    console.log("");
...
async function main() {
  ...
  console.log(
    `연결 테스트:        ${results.connection ? "✅ 통과" : "❌ 실패"}`
  );
  console.log(`CRUD 테스트:        ${results.crud ? "✅ 통과" : "❌ 실패"}`);
  console.log(
    `트랜잭션 테스트:    ${results.transaction ? "✅ 통과" : "❌ 실패"}`
  );
  console.log(
    `에러 처리 테스트:  ${results.errorHandling ? "✅ 통과" : "❌ 실패"}`
  );
  console.log("");
