...
async function main() {
  ...
  console.log(`   SUPABASE_SERVICE_ROLE_KEY: ${env.SUPABASE_SERVICE_ROLE_KEY ? '✅ 설정됨' : '❌ 누락'}`);
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message.includes('permission denied')) {
        ...
        console.log('   2. fix-supabase-permissions.sql 파일의 내용을 복사하여 실행');
        console.log('   3. 또는 다음 SQL을 직접 실행:');
        console.log('');
        console.log('      GRANT ALL ON ALL TABLES IN SCHEMA public TO postgres, anon, authenticated, service_role;');
        console.log('      GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.menus TO service_role;');
        console.log('      GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.orders TO service_role;');
        console.log('      GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.order_items TO service_role;');
        console.log('');
        console.log('   4. 권한 부여 후 다시 테스트:');
        console.log('      npm run debug:supabase');
        console.log('');
        console.log('   📝 참고: Prisma는 직접 DB에 연결하므로 정상 작동합니다.');
        console.log('      Supabase 클라이언트는 PostgREST API를 통해 접근하므로');
