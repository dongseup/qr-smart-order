...
async function main() {
  ...
  // 1. Prisma 연결 테스트
  results.push(
    await runTest('Prisma 연결 테스트', testPrismaConnection),
  );

  // 2. Supabase 연결 테스트 (선택적)
  const supabaseTestResult = await runTest('Supabase 연결 테스트', testSupabaseConnection);
  ...
  if (supabaseAvailable) {
    results.push(
      await runTest('스키마 일관성 테스트', testSchemaConsistency),
    );
  } else {
    results.push({
      name: '스키마 일관성 테스트',
  ...
  if (supabaseAvailable) {
    results.push(
      await runTest('데이터 일관성 테스트 (Prisma → Supabase)', testDataConsistency),
    );
  } else {
    results.push({
  ...
  if (supabaseAvailable) {
    results.push(
      await runTest('데이터 일관성 테스트 (Supabase → Prisma)', testSupabaseToPrisma),
    );
  } else {
    results.push({
  ...
  const prismaTests = results.filter((r) => 
    r.name === 'Prisma 연결 테스트' || 
    r.name === '성능 테스트'
  );
  const prismaAllPassed = prismaTests.length > 0 && prismaTests.every((r) => r.passed);
  ...
  if (prismaAllPassed) {
    ...
    if (supabaseAvailable) {
      ...
      console.log('   Prisma는 정상 작동하므로 데이터베이스 접근에는 문제가 없습니다.');
      console.log('');
      console.log('💡 Supabase 권한 문제 해결 방법:');
      console.log('   1. SUPABASE_SERVICE_ROLE_KEY 환경변수 확인');
      console.log('   2. Supabase 대시보드 > Settings > API에서 service_role key 확인');
      console.log('   3. Service Role Key는 RLS를 우회하므로 권한 문제가 없어야 합니다');
      console.log('   4. Prisma를 통한 데이터베이스 접근은 정상이므로 API 개발은 계속 진행 가능');
      console.log('');
